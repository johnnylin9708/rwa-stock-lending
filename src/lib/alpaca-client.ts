 ...
 * Reference: https://docs.alpaca.markets/reference/deleteallpositionsforaccount
 */
export async function closeAllPositions(alpacaAccountId: string) {
    const client = getBrokerApiClient();
    try {
        const response = await client.delete(`/v1/trading/accounts/${alpacaAccountId}/positions`);
        return response.data;
    } catch (error: any) {
        console.error(`Failed to close all positions for ${alpacaAccountId}:`, error.response?.data || error.message);
        throw error;
    }
}
 ...
 * Reference: https://docs.alpaca.markets/reference/deletepositionforaccount
 */
export async function closePosition(alpacaAccountId: string, symbol: string, qty?: number, percentage?: number) {
    const client = getBrokerApiClient();
    try {
        const params: any = {};
        ...
        const response = await client.delete(`/v1/trading/accounts/${alpacaAccountId}/positions/${symbol}`, { params });
        return response.data;
    } catch (error: any) {
        console.error(`Failed to close position ${symbol} for ${alpacaAccountId}:`, error.response?.data || error.message);
        throw error;
    }
}

/**
 * Place an order for a specific customer account
 */
export async function placeOrder(alpacaAccountId: string, orderParams: {
    symbol: string;
    qty?: number;
    notional?: number;
    side: "buy" | "sell";
    ...
    try {
        const response = await client.post(
            `/v1/trading/accounts/${alpacaAccountId}/orders`,
            orderParams
        );
        return response.data;
    } catch (error: any) {
        console.error(`Failed to place order for ${alpacaAccountId}:`, error.response?.data || error.message);
        throw error;
    }
}
 ...
 */
export async function getOrders(alpacaAccountId: string, status?: 'open' | 'closed' | 'all') {
    const client = getBrokerApiClient();
    try {
        const params = status ? { status } : {};
        const response = await client.get(`/v1/trading/accounts/${alpacaAccountId}/orders`, { params });
        return response.data;
    } catch (error: any) {
        console.error(`Failed to fetch orders for ${alpacaAccountId}:`, error.response?.data || error.message);
        throw error;
    }
}
...
export async function getHistoricalBars(
    ...
    try {
        ...
        const response = await client.get(`/v2/stocks/${symbol}/bars`, { params });
        return response.data;
    } catch (error: any) {
        console.error(`Failed to fetch bars for ${symbol}:`, error.response?.data || error.message);
        throw error;
    }
}
 ...
 */
export async function updateAccount(alpacaAccountId: string, updates: any) {
    const client = getBrokerApiClient();
    try {
        const response = await client.patch(`/v1/accounts/${alpacaAccountId}`, updates);
        return response.data;
    } catch (error: any) {
        console.error(`Failed to update account ${alpacaAccountId}:`, error.response?.data || error.message);
        throw error;
    }
}
...
export async function getAccountActivities(
    ...
    try {
        ...
        const response = await client.get(
            `/v1/accounts/${alpacaAccountId}/activities`,
            { params }
        );
        return response.data;
    } catch (error: any) {
        console.error(`Failed to fetch activities for ${alpacaAccountId}:`, error.response?.data || error.message);
        throw error;
    }
}
